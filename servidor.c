...
void serverUDP(int s, char * buffer, struct sockaddr_in clientaddr_in)
{
	...
	while (!final) 
	{
		...
		char *carriage_return = memchr(buffer, '\r', len);
		int message_length;

		if (newline != NULL || carriage_return != NULL)
		{
			message_length = (int)(newline != NULL ? newline - buffer : carriage_return - buffer);
		}			
		else 
		{
			const char *response = "500 Error de sintaxis\n";
		...
		// Almacenar el mensaje en la variable
        snprintf(mensaje, sizeof(mensaje), "%.*s", message_length, buffer);
