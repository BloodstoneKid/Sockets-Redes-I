...
void TCPCliente(char * hostname, char * fichero)
{
    ...
    while (fgets(buf, sizeof(buf), archivo)) 
	{
		...
		{
			fprintf(stderr, "%s: Connection aborted on error ",	"Cliente TCP");
	...
	while (i = recv(s, buf, TAM_BUFFER, 0)) 
	{
		...
		{
			printf("CLIENTE: No se encontró un carácter de nueva línea o retorno de carro");
		}
...
{
    int i, errcode;
	int retry = INTENTOS;		/* holds the retry count */
